  ...
  public async getDashboardSummary(userId: string) {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        if (possibleArray) {
          aiSuggestions = possibleArray as string[];
        }
      }
    } catch (err) {
      logger.error(`Failed to generate AI insights from Groq: ${err}`);
    }
    ...
    return {
      ...
      appointments: appointments.map((a) => ({
        id: a._id.toString(),
        doctorName: `Dr. ${(a.doctorId as any).firstName} ${(a.doctorId as any).lastName}`,
        specialty: (a.doctorId as any).specialty,
        startTime: a.startTime,
        status: a.status,
      })),
      medications: medicationLogs.map((m) => ({
        id: m._id.toString(),
        name: (m.reminderId as any)?.medicineName || 'Medication',
        dosage: (m.reminderId as any)?.dosage || '',
